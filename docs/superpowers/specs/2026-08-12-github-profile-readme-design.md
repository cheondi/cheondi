...
2026-08-12 로컬 네트워크 검사에서 `github-readme-stats` 공개 인스턴스는 네 개 카드 요청 모두 HTTP 503을 반환했다. `streak-stats` 공개 인스턴스도 같은 검사 안에서 정상 응답 후 15초 타임아웃이 재현됐다. 프로필에 깨진 이미지가 캐시되는 상황을 피하기 위해 통계, 주요 언어와 연속 활동 카드는 GitHub Actions가 정적 SVG로 생성해 `output` 브랜치에 게시하는 구조로 확정했다.
...
아래 카드를 같은 색상 파라미터로 맞추고 GitHub Actions에서 정적 SVG로 생성한다.

- `github-readme-stats-action`: 전체 통계
- `github-readme-stats-action`: 주요 언어
- `github-readme-streak-stats`: 연속 활동
- `github-readme-activity-graph`: 활동 그래프

정적 카드와 스네이크는 `output` 브랜치에 함께 게시한다. 활동 그래프만 정상 응답을 확인한 외부 SVG를 사용한다. `<picture>`와 `prefers-color-scheme` 소스를 사용해 GitHub 라이트·다크 테마에서 모두 읽히게 한다.
...
- 외부 동적 이미지 URL은 적용 전에 직접 요청해 정상 응답과 이미지 콘텐츠 유형을 확인한다.
- 통계와 연속 활동 공개 인스턴스는 런타임 의존에서 제외하고 Actions 생성형 정적 SVG를 사용한다.
